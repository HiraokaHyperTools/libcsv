    ...
    public class Csvr
    {
        ...
        public Csvr ReadStr(String text, char sep, char quote)
        {
            ...
            while (true)
            {
                ...
                if (res.HasFlag(ReadRes.DataRead))
                {
                    columns.Add(column);

                    if (res.HasFlag(ReadRes.Sep | ReadRes.EOF))
                    {
                        columns.Add("");
                    }
                }
                ...
                {
                    if (res.HasFlag(ReadRes.Sep))
                    {
                        columns.Add("");
                    }

                    if (res.HasFlag(ReadRes.EOF) || res.HasFlag(ReadRes.EOL))
                    {
                        if (columns.Count != 0)
                        {
                            columns.Add("");
                ...
                if (res.HasFlag(ReadRes.EOL))
                {
                    rows.Add(columns.ToArray());
                    columns.Clear();
                }
                else if (res.HasFlag(ReadRes.EOF))
                {
                    if (columns.Count != 0)
                    {
                        rows.Add(columns.ToArray());
